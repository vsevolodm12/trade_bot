...
## Баг-фиксы (2026-03-01) — ожидают деплоя

- [x] **Акция не добавлялась на экран**: `alertAdded` теперь явно вызывает HTMX-рефреш `#alerts-wrap` после закрытия шита (failsafe поверх inline swap) — `web/templates/base.html`
- [x] **Кнопка «Назад» со страницы поиска**: при нахождении акции теперь скрывается поисковая форма → пользователь видит только хедер с «Назад» (закрывает шит) и карточку — `web/templates/partials/add_form.html`
- [x] **Наложение главной страницы при скролле в шите**: добавлена блокировка `touchmove` на body пока шит открыт (iOS bounce fix) — `web/templates/base.html`
- [x] **График Тенсента (HK)**: `_tradingview_url` теперь конвертирует `0700.HK` → `HKEX-700` (убирает `.HK` и ведущие нули) — `web/app.py`, `web/templates/partials/alert_list.html`
- [x] **Счётчик уведомлений**: `total` теперь = `len(alerts)` (реальные алерты), а не `len(grouped)` (карточки); счётчик также обновляется при авторефреше через HTMX OOB swap — `web/app.py`, `web/templates/partials/alert_list.html`, `web/templates/index.html`
